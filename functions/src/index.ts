...
const resend = new Resend(
  functions.config().resend?.api_key ||
    ...
    "re_Wz5buHio_4diMFr7nDkwBRSGfy31kqi4b"
);
...
export const sendEmail = functions.https.onRequest((request, response) => {
  return corsHandler(request, response, async () => {
    ...
    try {
      const { name, email, phone, industry, product, requirement, to_email } =
        request.body;
      ...
      if (!name || !email || !phone || !industry || !requirement) {
        response.status(400).json({
          success: false,
          message:
            "Missing required fields: name, email, phone, industry, and requirement are required",
        });
        return;
      }
      ...
      if (error) {
        ...
        if (error.message && error.message.includes("domain is not verified")) {
          errorMessage = `Domain verification issue. Please verify the API key matches the Resend account where duramettechnologies.com is verified.`;
        }
